...
const ComplianceFormON = () => {
  ...
  const populateTable = () => {
    ...
    if (getSelectpicker) {
      Array.from(getSelectpicker).forEach((element) => {
        element.style.borderRadius = '10px';
        console.log("aaaaaaa22222222222222");
        window.$(element).selectpicker();
      });
    }
  ...
  const getCompanyinfo = () => {
    ...
    axios.get(`/companyinfo/${getUserSID()}`).then((res) => {
      const { City, Country } = res.data.data.body.legaladdress;
      ...
      <div className="row">
        ...
        <div className="col-3 d-flex gap-3 flex-column position-fixed end-0" style={{ marginTop: '173px' }}>
          ...
          <div className="d-flex gap-4">
            ...
            <button className="btn btnPrimary btn-sm mb-2" disabled={!(sectionA.task_approverer_signoff && sectionA.task_preparer_signoff)} onClick={handleDownloadClick}>
              Print/Download
            </button>
          </div>
