...
import { changeReportIncompleteToComplete } from "./calculator/screen4/Screen4";
...
const ReportsPage = ({ currentUserRole }) => {
  ...
  const [allReports, setAllReports] = useState({ data: [] });
  ...
  useEffect(() => {
      ...
      if (querySearch === "") {
        ...
        try {
          const reports = await axios.get(
            `/reports/${getUserSID()}?maxNumber=200`
          );
          setLoading(false)
          if (reports) {
            ...
            setAllReports(reports.data.data.body);
    ...
    fetchReports();
  }, [handleRefreshClick, pageNumber, currentUserRole, renderPageCount]);
  ...
  }, [allInputChecked, inputsChecked, currentUserRole]);

  useEffect(() => {
    const filteredData = allReports?.data?.filter((item) => {
  ...
  const sliceData = () => {
    return allReports?.data?.slice(0, rowsPerPage);
  };
    ...
    if (querySearch === "" || querySearch === null) {
      ...
      axios
        ...
        .then((res) => {
          ...
          console.log("res", res);
          if (res.data.data.code === 200) {
            setAllReports(res.data.data.body);
          }
        })
        .catch((err) => {
          setLoading(false)
      ...
      <div className="panel trans">
        ...
        <div className="pBody">
          ...
          <div className="tableOuter d-none">
            {reportsLoaded && allReports.data.length >= 1 ? (
              <table className="table customGrid">
                <thead>
                  <tr>
                    <th className="tdCheckBox">
            ...
            ) : reportsLoaded && allReports.data.length === 0 ? (
